...
module.exports = function(options) {
  ...
  var cachableCommands = 'help,ls,search,uimage,uinfo'.split(',');

  /**
   * Add cache control to cachable spop commands
   *
  ...
  function cache(req, res, next) {
    var parts = unescape(req.path.substr(1)).split(/(\/|\s)/),
        command = parts[0];

    if (cachableCommands.indexOf(command) < 0) {
      return next();
    } else if (command === 'uimage') {
